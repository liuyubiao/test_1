...
@software: PyCharm 
@file: category_128.py
@time: 2022-06-05 17:44 
@description: 针对128-酱油进行处理
"""
import re
import requests
import json
...
def fromat_attribute(product_dict, image_path_list):
    ...
    :return:
    '''
    brand_list, weight_list, product_name_list, package_list, type_list, add_list, type2_list = product_dict['品牌1'], product_dict['重容量x数量'], product_dict['商品全称'], product_dict['包装形式'],product_dict['类型'],product_dict['含有物'],product_dict['类型2']
    # 去重
    brand_list, weight_list, product_name_list, package_list, type_list, add_list, type2_list = remove_format(brand_list), remove_format(weight_list),remove_format(product_name_list),remove_format(package_list),remove_format(type_list),remove_format(add_list),remove_format(type2_list)
    ...
    # 类型2
    type2 = type2_deal(type2_list)

    # 商品全称二次处理
    if product_name == '不分':
        # 使用含有物+ 类型 拼接出商品全称
        if add != '不分':
            product_name = '{}{}'.format(add, type)
        else:
            product_name = type

    content = '{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(product_dict['品类编码'], product_dict['品类名称'],
                                                                    product_dict['商品编码'], brand1, brand2, weight_num,
                                                                    weight, product_name, package, type, add, type2)
...
def get_url_result_pre(imageList,url,category_id="128"):
    package_list = []
...
def get_url_result(imageList,url,category_id="128"):
    package_list = []
    for imagePath in imageList:
        payload = {
            "data": imagePath
...
def package_deal(package_list, image_path_list):
    ...
    '''
    package_type = '玻璃瓶'
    rule_package_list = package_list[0]

    if len(rule_package_list) > 0:
        package_type = rule_package_list[0]
    else:
        # 获取材质
...
def type_deal(type_list):
    '''
    类型
    2022-08-16进行修改：将类型完全按照人工操作的规范修正
    :param type_list:
    :return:
    '''
    ...
    sort_type_list = ['蚝油', '老抽', '味极鲜', '生抽', '鱼露', '鲜味汁', '鲜味液', '鲜味露', '其他汁', '其他露','其他液', '鲜酱油', '酱油']

    if len(rule_type_list) > 0:
        for item in sort_type_list:
            if item in rule_type_list:
                type = item
    else:
...
def add_deal(add_list):
    ...
    if len(rule_add_list) > 0:
        add = max(rule_add_list, key=len)
    else:
        if len(uie_add_list) > 0:
            add = max(uie_add_list, key=len)
    return add

def type2_deal(type_list):
    '''
    类型2
    :param type_list:
    :return:
    '''
    rule_type2_list, uie_type2_list = type_list[0], type_list[1]
    type2 = '不分'

    if len(rule_type2_list) > 0:
        # 对添加剂进行判断
        if '有添加' in rule_type2_list:
            if '无添加' in rule_type2_list or '有机' in rule_type2_list:
                rule_type2_list.remove('有添加')
        type2 = '，'.join(rule_type2_list)
    else:
        if len(uie_type2_list) > 0:
            type2 = '，'.join(uie_type2_list)
    return type2
...
def deal_result_format(product_dict, result_list, image_list):
    ...
    for rule_index, rule_result in enumerate(result_list):
        ...
        package_list, type_list, add_list, type2_list = infos['info1'], infos['info2'], infos['info3'], infos['info4']
        ...
        if rule_index == 0:
            ...
            result_type2_list = type2_list
        else:
            [(brand.extend(brand_list[index]), weight.extend(weight_list[index]),
              product_name.extend(product_name_list[index]), package.extend(package_list[index]),
              type.extend(type_list[index]), add.extend(add_list[index]), type2.extend(type2_list[index])) for
             index, (brand, weight, product_name, package, type, add, type2) in
             enumerate(zip(result_brand1_list, result_weight_num_list, result_product_name_list, result_package_list,
                           result_type_list, result_add_list, result_type2_list))]
    ...
    product_dict['类型2'] = result_type2_list

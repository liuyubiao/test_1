...
@software: PyCharm 
@file: category_129.py
@time: 2022-06-05 17:44 
@description: 针对129-果冻布丁(嗜喱)进行处理
"""
import re
import requests
import json
...
def fromat_attribute(product_dict, image_path_list):
    ...
    :return:
    '''
    brand_list, weight_list, product_name_list, taste_list,package_list = product_dict['品牌1'], product_dict['重容量x数量'], product_dict['商品全称'], product_dict['口味'],product_dict['包装形式']
    # 去重
    brand_list, weight_list, product_name_list,taste_list,package_list = remove_format(brand_list), remove_format(weight_list),remove_format(product_name_list),remove_format(taste_list),remove_format(package_list)
    ...
    # 商品全称二次处理
    if product_name == '果冻' or product_name == '布丁':
        # 使用口味+商品全称 拼接出商品全称
        if taste != '不分':
            product_name = '{}{}'.format(max(taste.split('、'),key=len), product_name)

    content = '{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(product_dict['品类编码'], product_dict['品类名称'],
                                                                    product_dict['商品编码'], brand1, brand2, weight_num,
                                                                    weight, product_name, taste,package)
    return content
...
def product_name_deal(product_name_list):
    ...
    product_name = rule_product_name_list[0]

    if len(rule_product_name_list) > 0 and rule_product_name_list[0] not in ['果冻', '布丁']:
...
def get_url_result_pre(imageList,url,category_id="129"):
    package_list = []
...
def get_url_result(imageList,url,category_id="129"):
    package_list = []
    for imagePath in imageList:
        payload = {
            "data": imagePath
...
def deal_result_format(product_dict, result_list, image_path_list):
    ...
    for rule_index, rule_result in enumerate(result_list):
        ...
        taste_list, package_list = infos['info1'], infos['info2']
        ...
        if rule_index == 0:
            ...
            [(brand.extend(brand_list[index]), weight.extend(weight_list[index]),
              ...
              package.extend(package_list[index])) for
             index, (brand, weight, product_name, taste, package) in

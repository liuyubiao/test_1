...
@software: PyCharm 
@file: category_133.py
@time: 2022-06-05 17:44 
@description: 针对133-面包进行处理
"""
import re
import requests
import json
...
def fromat_attribute(product_dict,image_path_list):
    ...
    :return:
    '''
    brand_list, weight_list, product_name_list, toast_list, taste_list,type_list,stuff_list,time_list = product_dict['品牌1'], product_dict['重容量x数量'], product_dict['商品全称'], product_dict['现烤/非现烤'],product_dict['口味'],product_dict['类型'],product_dict['馅料'],product_dict['保质期']
    # 去重
    brand_list, weight_list, product_name_list, toast_list, taste_list,type_list,stuff_list,time_list = remove_format(brand_list), remove_format(weight_list),remove_format(product_name_list),remove_format(toast_list),remove_format(taste_list),remove_format(type_list),remove_format(stuff_list),remove_format(time_list)
    ...
    # 类型
    type = type_deal(type_list,weight,stuff_list)
    # 馅料
    stuff = stuff_deal(stuff_list)
    # 保质期
    ...
    if product_name == '不分':
        ...
        if taste != '不分':
            product_name = '{}{}'.format(taste, bread_type)
        else:
            product_name = bread_type

    content = '{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(product_dict['品类编码'], product_dict['品类名称'],
                                                                    product_dict['商品编码'], brand1, brand2, weight_num,
                                                                    weight, product_name, toast,taste,type,stuff,time)
    return content
...
def product_name_deal(product_name_list):
    ...
    if len(uie_product_name_list) > 0:
        ...
        if len(product_name) < 4 or (product_name.find('面包') < 0 and product_name.find('吐司') < 0):
            product_name = '不分'
    return product_name
...
def type_deal(type_list, weight, stuff_list):
    '''
    类型
    :param type_list:
    :param weight:
    ...
    if len(rule_type_list) > 0:
        type = max(rule_type_list,key=len)
    else:
        if len(stuff_list[0]) > 0:
            ...
            type = '无馅小包'
            if weight != '不分':
                weight_g = re.findall("\d+", weight)[0]
                if float(weight_g) > 100:
                    type = '无馅大包'
    return type

def stuff_deal(stuff_list):
    '''
    馅料
    :param stuff_list:
    :return:
    '''
    rule_stuff_list, uie_stuff_list = stuff_list[0], stuff_list[1]

    stuff = '不分'
    if len(rule_stuff_list) > 0:
        stuff = '、'.join(rule_stuff_list)
    else:
        if len(uie_stuff_list) > 0:
            stuff = '、'.join(uie_stuff_list)
    return stuff
...
def deal_result_format(product_dict, result_list, image_list):
    '''
    将返回的结果进行格式化操作
    :param product_dict:
    :param result_list:
    ...
    for rule_index, rule_result in enumerate(result_list):
        ...
        toast_list, taste_list,type_list,stuff_list,time_list = infos['info1'], infos['info2'],infos['info3'], infos['info4'],infos['info5']
        ...
        if rule_index == 0:
            ...
            result_toast_list = toast_list
            result_taste_list = taste_list
            result_type_list = type_list
            result_stuff_list = stuff_list
            result_time_list = time_list
        else:
            [(brand.extend(brand_list[index]), weight.extend(weight_list[index]),
              product_name.extend(product_name_list[index]), toast.extend(toast_list[index]),taste.extend(taste_list[index]),
              type.extend(type_list[index]), stuff.extend(stuff_list[index]), time.extend(time_list[index])) for
             index, (brand, weight, product_name, toast, taste, type, stuff, time) in
             enumerate(zip(result_brand1_list, result_weight_num_list, result_product_name_list, result_toast_list,
                           result_taste_list, result_type_list, result_stuff_list, result_time_list))]

    product_dict['品牌1'] = result_brand1_list
    product_dict['重容量x数量'] = result_weight_num_list
    product_dict['商品全称'] = result_product_name_list

    product_dict['现烤/非现烤'] = result_toast_list
    product_dict['口味'] = result_taste_list
    product_dict['类型'] = result_type_list
    product_dict['馅料'] = result_stuff_list
    product_dict['保质期'] = result_time_list

    # 将结果进行整体的整合处理
    content = fromat_attribute(product_dict, image_list)
    return content

...
@software: PyCharm 
@file: category_134.py
@time: 2022-06-05 17:44 
@description: 针对134-蛋糕进行处理
"""
import re
import requests
import json
...
def fromat_attribute(product_dict, image_path_list):
    ...
    :return:
    '''
    brand_list, weight_list, product_name_list, taste_list,add_list,time_list,type_list = product_dict['品牌1'], product_dict['重容量x数量'], product_dict['商品全称'], product_dict['口味'],product_dict['含有物'],product_dict['保质期'],product_dict['类型']
    # 去重
    brand_list, weight_list, product_name_list, taste_list,add_list,time_list,type_list = remove_format(brand_list), remove_format(weight_list),remove_format(product_name_list),remove_format(taste_list),remove_format(add_list),remove_format(time_list),remove_format(type_list)
    ...
    content = '{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n'.format(product_dict['品类编码'], product_dict['品类名称'],
                                                                    ...
                                                                    weight, product_name, taste,add,time,type)
    return content
...
def type_deal(type_list):
    ...
    rule_type_list = [item for item in rule_type_list if item != '蛋糕']
    ...
    if len(rule_type_list) > 0:
        ...
        if len(uie_type_list) > 0:
            type = max(uie_type_list, key=len)
    return type
...
def deal_result_format(product_dict, result_list, image_path_list):
    ...
    for rule_index, rule_result in enumerate(result_list):
        ...
        taste_list,add_list,time_list,type_list = infos['info1'], infos['info2'],infos['info3'], infos['info4']
        ...
        if rule_index == 0:
            ...
            [(brand.extend(brand_list[index]), weight.extend(weight_list[index]),
              product_name.extend(product_name_list[index]), taste.extend(taste_list[index]),
              add.extend(add_list[index]), time.extend(time_list[index]),
              type.extend(type_list[index])) for
             index, (brand, weight, product_name, taste, add, time, type) in
             enumerate(zip(result_brand1_list, result_weight_num_list, result_product_name_list, result_taste_list,
                           result_add_list, result_time_list, result_type_list))]
